...
class DialogueAgent:
    ...
    def _evaluate_child_answer_with_llm(
        ...
        if stage == Stage.S1_EMOTION_LABELING:
            ...
            [성공 조건 - 하나라도 충족하면 성공]
            1. 명확한 경험 유무 답변: "있어", "없어", "본 적 있어", "없었어"
            2. 구체적인 경험 설명: "친구가 ~했어", "학교에서 ~봤어", "어제 ~일어났어"
            
            [성공 예시]
            - "있어요" → 성공 (명확한 긍정)
            - "없어" → 성공 (명확한 부정)
            - "친구가 혼자 있었어" → 성공 (구체적 경험)
            - "학교에서 봤어요" → 성공 (구체적 경험)
            - "어제 친구가 울었어" → 성공 (구체적 경험)
            
            [실패 예시]
            - "속상했을 것 같아" → 실패 (경험 유무가 아닌 감정 추측)
            - "슬펐을 거야" → 실패 (경험 유무가 아닌 감정 표현)
            - "힘들었을 것 같아요" → 실패 (감정 추측)
            - "~했을 것 같아" 형식 → 실패 (추측 표현)
            - "몰라", "글쎄" → 실패 (회피)
            - "아마도", "~같아" → 실패 (불확실한 추측)
            ...
            중요: 
            - "~했을 것 같아" 패턴은 추측이므로 실패!
            - 감정 단어만 있으면 실패!
            - 반드시 경험 유무(있다/없다) 또는 구체적 경험 설명이 있어야 성공!
            """
        elif stage == Stage.S4_REAL_WORLD_EMOTION:
