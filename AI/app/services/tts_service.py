...
class TTSService:
    ...
    def get_voice_id(self, voice_name: str = "Anna") -> str:
        """
        보이스 이름으로 voice_id 조회
        
        Args:
            voice_name: 사용할 보이스 이름 (기본값: "Anna" - 어린 여자 아이 목소리)
        
        Returns:
            voice_id: Supertone voice ID
        """
        # 캐싱된 voice_id가 있으면 재사용
        if self._default_voice_id and voice_name == "Anna":
    ...
    def text_to_speech(
        self,
        text: str,
        voice_name: str = "Anna",  # 어린 여자 아이 목소리 (child, female)
        language: str = "ko",
        style: str = "neutral",
